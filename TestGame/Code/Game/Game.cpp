...
void Game::StartUp()
{
	...
	m_player->m_position = Vec3( 0.0f, 0.0f, 20.0f );
...
//----------------------------------------------------------------------------------------------------------------------
void Game::DrawJobStatusBlocks() const
{
	std::vector<Vertex_PCU> verts;

	for ( int i = 0; i < m_allTestJobs.size(); i++ )
	{
		TestJob* job = m_allTestJobs[i];
		AABB2 tileBounds = GetTestJobTileBounds( job->m_tileCoords );
		Rgba8 tileColor  = GetTestJobTileColor( job->m_jobStatus );
		AddVertsForAABB2D( verts, tileBounds, tileColor );
	}

	Mat44 matrix;
	matrix.SetTranslation3D( Vec3( 0.0f, 0.0f, 10.0f ) );
...
void Game::CreateTestJobs()
{
	...
	for ( int tileY = 0; tileY < 20; tileY++ )
	{
		...
		{
			int sleepMS		 = rng.RollRandomIntInRange( 50, 3000 );
			TestJob* testJob = new TestJob( tileX, tileY, sleepMS );
			m_allTestJobs.push_back( testJob );
			g_theJobSystem->PostNewJob( testJob );
		}
	}
}
...
void Game::UpdateTestJobs()
{
	...
	{
		Job* retrievedJob = g_theJobSystem->RetrieveCompletedJob();
	}
...
//----------------------------------------------------------------------------------------------------------------------
AABB2 Game::GetTestJobTileBounds( IntVec2 tileCoords ) const
{
	float paddingAroundMe	= 0.1f;
//	Vec2 mins				= Vec2( tileCoords.x + paddingAroundMe, tileCoords.y + paddingAroundMe );
//	Vec2 maxs				= Vec2( tileCoords.x - paddingAroundMe, tileCoords.y - paddingAroundMe );
//	AABB2 tileBounds		= AABB2( mins, maxs );
	
	AABB2 tileBounds		= AABB2( Vec2( tileCoords.x, tileCoords.y ), Vec2( tileCoords.x + 1, tileCoords.y + 1 ) );
	tileBounds.AddPadding( paddingAroundMe, paddingAroundMe );
	return tileBounds;
}

//----------------------------------------------------------------------------------------------------------------------
Rgba8 Game::GetTestJobTileColor( JobStatus jobStatus ) const
{
	switch( jobStatus )
	{
		case JOB_STATUS_NEW:		return Rgba8::DARK_RED;
		case JOB_STATUS_QUEUED:		return Rgba8::RED;
		case JOB_STATUS_WORKING:	return Rgba8::YELLOW;
		case JOB_STATUS_COMPLETED:	return Rgba8::GREEN;
		case JOB_STATUS_RETRIEVED:	return Rgba8::BLUE;
		default: ERROR_AND_DIE( "INVALID Test job tile color" );
	}
}
