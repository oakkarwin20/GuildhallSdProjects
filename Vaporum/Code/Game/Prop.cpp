...
void Prop::AddGridLinesToScene()
{
	...
	for ( int i = -50; i < 50; i++ )
	{
		...
		AABB3 x_bounds = AABB3( -50.0f, -0.02f, -0.08f, 50.0f, 0.02f, -0.04f );
		x_bounds.m_mins.y += i;
		x_bounds.m_maxs.y += i;
	...
	for ( int j = -50; j < 50; j++ )
	{
		...
		AABB3 y_bounds = AABB3( -0.02f, -50.0f, -0.18f, 0.02f, 50.0f, -0.14f );
		y_bounds.m_mins.x += j;
		y_bounds.m_maxs.x += j;

		// X Line is at origin, set color = brightest Green
		if ( j == 0 )
		{
