...
void Prop::Render() const
{
	...
	g_theRenderer->SetModelConstants( GetModelMatrix( m_position, m_orientationDegrees ), m_color );
	g_theRenderer->DrawVertexArray( static_cast<int>( m_verts_PCU.size() ), m_verts_PCU.data() );
...
void Prop::AddCubeToScene( float x, float y, float z )
{
	...
	topRight	= Vec3(  x,  y,  z );
	topLeft		= Vec3(  x, -y,  z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::RED );
	...
	topRight	= Vec3( -x, -y,  z );
	topLeft		= Vec3( -x,  y,  z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::CYAN );
	...
	topRight	= Vec3( -x, y,  z );
	topLeft		= Vec3(  x, y,  z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::GREEN );
	...
	topRight	= Vec3(  x, -y,  z );
	topLeft		= Vec3( -x, -y,  z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::MAGENTA );
	...
	topRight	= Vec3(  x,  y, z );
	topLeft		= Vec3( -x,  y, z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::BLUE );
	...
	topRight	= Vec3( -x,  y, -z );
	topLeft		= Vec3(  x,  y, -z );
	AddVertsForQuad3D( m_verts_PCU, bottomLeft, bottomRight, topRight, topLeft, Rgba8::YELLOW );
}
...
{
	m_texture = texture;
	AddVertsForSphere3D( m_verts_PCU, sphereCenter, sphereRadius, sphereNumSlices, sphereNumStacks );
}
...
void Prop::AddGridLinesToScene()
{
	...
	for ( int i = -50; i < 50; i++ )
	{
		...
		AddVertsForAABB3D( m_verts_PCU, x_bounds, x_axisColor );
	}
	...
	for ( int j = -50; j < 50; j++ )
	{
		...
		AddVertsForAABB3D( m_verts_PCU, y_bounds, y_axisColor );
	}
}
